import React, { useState } from 'react';
import { Check, Music2, Instagram, Sparkles, Command } from 'lucide-react';
import { useDownload } from '../hooks/useDownload';
import { useAnalytics } from '../lib/analytics';
import CommentPreview from './ui/CommentPreview';
import ControlsPanel from './ui/ControlsPanel';
import StickerFrame from './ui/StickerFrame';
import { motion } from 'motion/react';

export default function InstagramGenerator() {
  const [username, setUsername] = useState('ugc_creator');
  const [comment, setComment] = useState('This product literally changed my life! Highly recommend it to everyone. 😍');
  const [profilePic, setProfilePic] = useState('https://picsum.photos/seed/avatar1/200/200');
  const [isVerified, setIsVerified] = useState(true);
  const [theme, setTheme] = useState<'light' | 'dark'>('light');
  const [replyingTo, setReplyingTo] = useState('');

  const { trackEvent } = useAnalytics();
  const { downloadRef, download } = useDownload();

  const handleDownload = async () => {
    trackEvent('conversion', { tool: 'instagram', action: 'download' });
    download('instagram-comment');
  };

  const reset = () => {
    setUsername('ugc_creator');
    setComment('This product literally changed my life! Highly recommend it to everyone. 😍');
    setProfilePic('https://picsum.photos/seed/avatar1/200/200');
    setIsVerified(true);
    setTheme('light');
    setReplyingTo('');
  };

  return (
    <div className="bg-[#030303] min-h-screen pt-32 pb-24 px-6 sm:px-10 overflow-hidden">
      <div className="mx-auto max-w-7xl">
        <header className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-pink-500/10 border border-pink-500/20 rounded-full">
              <Instagram size={14} className="text-pink-500" />
              <span className="text-[10px] font-black uppercase tracking-widest text-pink-500">Asset Sub-system 01</span>
            </div>
            <h1 className="font-display text-4xl sm:text-6xl font-bold tracking-tight text-white leading-tight">
              Instagram <br/>
              <span className="text-gradient">Comment Studio</span>
            </h1>
            <p className="text-lg text-white/40 font-medium max-w-xl">
              Industrial-grade Instagram comment generation. High precision layouts designed for viral UGC production.
            </p>
          </div>

          <div className="flex items-center gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/5">
             <div className="px-4 py-2 bg-white text-black rounded-xl text-xs font-bold">Standard Layout</div>
             <div className="px-4 py-2 text-white/40 text-xs font-bold hover:text-white/60 transition-colors cursor-pointer">Advanced (Coming)</div>
          </div>
        </header>

        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 items-start">
          {/* Controls Panel */}
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className="lg:col-span-5"
          >
            <ControlsPanel 
              onReset={reset}
              onDownload={handleDownload}
              fields={[
                { label: 'Profile Picture', type: 'upload', value: profilePic, onChange: setProfilePic },
                { 
                  label: 'Username', 
                  type: 'text', 
                  value: username, 
                  maxLength: 30,
                  onChange: setUsername
                },
                { label: 'Replying to', type: 'text', value: replyingTo, onChange: setReplyingTo },
                { label: 'Comment Body', type: 'textarea', value: comment, onChange: setComment },
                { 
                  label: 'Badge Status', 
                  type: 'toggle', 
                  value: isVerified, 
                  onChange: setIsVerified,
                  options: [
                    { label: 'None', value: false },
                    { 
                      label: 'Verified', 
                      value: true, 
                      icon: (
                        <div className="w-3.5 h-3.5 bg-[#0095f6] rounded-full flex items-center justify-center text-white">
                          <Check size={8} strokeWidth={5} />
                        </div>
                      ) 
                    }
                  ]
                },
                {
                  label: 'UI Theme',
                  type: 'toggle',
                  value: theme,
                  onChange: setTheme,
                  options: [
                    { label: 'System Light', value: 'light' },
                    { label: 'System Dark', value: 'dark' }
                  ]
                }
              ]}
            />
          </motion.div>

          {/* Preview Panel */}
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            className="lg:col-span-7 lg:sticky lg:top-32"
          >
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Real-time Preview Engine</span>
              </div>
              <div className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Alpha 1.2.0</div>
            </div>
            
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-accent to-pink-500 rounded-[48px] blur-2xl opacity-10 group-hover:opacity-20 transition-opacity duration-1000" />
              
              <div className="relative flex items-center justify-center min-h-[500px] glass rounded-[48px] border border-white/5 p-4 sm:p-12 overflow-hidden bg-dot-pattern">
                <StickerFrame
                  innerRef={downloadRef}
                  theme={theme}
                  platform="instagram"
                >
                  <CommentPreview 
                    platform="instagram"
                    username={username}
                    comment={comment}
                    profilePic={profilePic}
                    isVerified={isVerified}
                    theme={theme}
                    replyingTo={replyingTo}
                  />
                </StickerFrame>
              </div>
            </div>

            <div className="mt-8 flex gap-6">
               <div className="flex-1 glass rounded-3xl p-6 border-white/5">
                  <div className="flex items-center gap-2 mb-2">
                    <Sparkles size={14} className="text-accent" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/60">Deployment Tip</span>
                  </div>
                  <p className="text-xs text-white/30 leading-relaxed font-medium">
                    Exports are rendered with perfect alpha transparency. Import directly into CapCut or Premiere for authentic overlay.
                  </p>
               </div>
               <div className="flex-1 glass rounded-3xl p-6 border-white/5">
                  <div className="flex items-center gap-2 mb-2">
                    <Command size={14} className="text-white/60" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/60">Platform Compliance</span>
                  </div>
                  <p className="text-xs text-white/30 leading-relaxed font-medium">
                    Layout dimensions and fonts are synced with the latest Q1 2026 Instagram UI specifications.
                  </p>
               </div>
            </div>
          </motion.div>
        </div>
      </div>
    </div>
  );
}
