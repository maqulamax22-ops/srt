...
import { Plus, Edit2, Trash2, Save, X, Eye, EyeOff, LogIn, LogOut, LayoutDashboard } from 'lucide-react';
    ...
    if (!authLoading && user && isAdmin) {
      ...
      unsubscribePosts = onSnapshot(q, (snapshot) => {
        ...
        if (error.message.includes('permission')) {
          setPermissionError(`Firestore Permission Denied: Your account (${user.email}) is not authorized by the security rules.`);
        }
        setLoading(false);
      });
    } else if (!authLoading) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message?.includes('size')) {
        ...
        alert("PERMISSION ERROR: You are not authorized to save this post.");
      } else {
          ...
          <div className="space-y-2">
            ...
            {permissionError ? (
              ...
              <p className="text-slate-500 font-light leading-relaxed">
                Please sign in with an authorized administrator account to manage the community blog.
              </p>
            )}
          </div>
          <button
